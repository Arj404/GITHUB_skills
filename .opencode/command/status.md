...
1. **Read the Spec**: Check if `.copilot/spec/$ARGUMENTS.md` exists. Extract the `status` from its YAML frontmatter.
2. **Check Artifacts**: Check if the following directories/files exist in `.copilot/artifact/$ARGUMENTS/`:
...
## Next Steps
{Recommend the next agent to run based on what is missing or in-progress. E.g., "Run /code $ARGUMENTS to implement the plan."}
```
