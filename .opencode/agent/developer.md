...
- Read the plan from `.copilot/artifact/<spec_id>/plan/` or from the conversation.
- Reference the spec (`.copilot/spec/`) and research (`.copilot/artifact/<spec_id>/research/`) if they exist.
- Use `grep` and `read` tools to explore the codebase and identify the files, modules, and interfaces to modify or create.
- Use the `question` tool if requirements are ambiguous.
...
- Run unit tests via `bash` tool and ensure they pass.
- Run linters and formatters for the changed file types.
- Use the `lsp` tool to verify no new errors were introduced.
