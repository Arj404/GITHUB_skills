---
description: Transforms rough ideas into structured specifications (features, bugs, or spikes)
mode: primary
...
Use the `task` tool to autonomously research the codebase:
...
**If the scope looks like it should be split** into multiple stories:
...
2. **If Split:**
   - Create each story as a separate file: `.copilot/spec/<spec_id>-001.md`, etc.
   ...
   - After all stories are approved, suggest next steps once.
...
- Ask explicitly using the `question` tool:
...
- **If Approved:**
  ...
  4. **When all stories are approved** (or single story approved):
     - Suggest next steps:
       - **Research Technical Options** → run `/research <spec_id>` if the feature needs technology evaluation (new tech stack, DB choice, etc.).
       - **Plan Implementation** → run `/plan <spec_id>` if tech choices are known and the feature can go directly to planning.
